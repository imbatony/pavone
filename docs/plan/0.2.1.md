...
- [ ] **创建选项组装饰器**
  - `@common_download_options`: 下载相关选项
  - `@common_network_options`: 网络相关选项（proxy, header, timeout）
  - `@common_output_options`: 输出相关选项
...
- **功能**:
  - 提取 Open Graph 元数据 (og:title, og:image, og:description 等)
  - 提取标准 HTML meta 标签
  - 支持自定义正则表达式提取
  - 统一的异常处理
- **受益插件**: 所有HTML解析插件 (6+)
...
- **目标**: `ppvdatabank_metadata.py` 和 `supfc2_metadata.py` 共享FC2相关逻辑
...
- **目标**: 统一处理stream item、cover item、metadata item的创建和组装
...
class OperationItemBuilder:
    ...
    def add_stream(self, url: str, quality: str, **kwargs) -> "OperationItemBuilder":
        """添加流媒体项"""
        pass
    
    def set_cover(self, url: str) -> "OperationItemBuilder":
        """设置封面"""
        pass
    
    def set_metadata(self, metadata: MovieMetadata) -> "OperationItemBuilder":
        """设置元数据"""
        pass
...
### 5.3 第三步: 重构现有插件 - 阶段1 (1-2周)
- [x] 重构 `missav_plugin.py`
- [x] 重构 `av01_plugin.py`
- [x] 重构 `jtable.py`
- [x] 重构 `memojav.py`
- [x] 重构 `ppvdatabank_metadata.py`
- [x] 重构 `supfc2_metadata.py`
- [x] 确保所有现有测试通过
...
| 阶段 | 任务 | 预计时间 | 状态 |
|------|------|---------|------|
| Week 1 | 创建工具类 (HTMLMetadataExtractor, MetadataBuilder) | 5-7 天 | ⏳ 待开始 |
| Week 2 | 创建基类和构建器 (FC2BaseMetadata, OperationItemBuilder) | 5-7 天 | ⏳ 待开始 |
| Week 3-4 | 重构现有插件 - 阶段1 | 7-10 天 | ⏳ 待开始 |
| Week 4 | 深度优化 - 阶段2 | 3-5 天 | ⏳ 待开始 |
| Week 5 | 测试和验证 | 3-5 天 | ⏳ 待开始 |
| Week 5 | 文档更新和发布准备 | 2-3 天 | ⏳ 待开始 |
