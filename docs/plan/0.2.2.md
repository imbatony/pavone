...
**文件**: `pavone/plugins/extractors/jtable.py` → `pavone/plugins/jtable_plugin.py`
...
- **实现接口**:
  - ✅ `BasePlugin` (基础插件)
  - ✅ 元数据提取功能（can_extract, extract_metadata）
  - ✅ 视频下载功能（can_handle, extract）
- **代码迁移**:
  - ✅ 从 `extractors/jtable.py` 迁移元数据提取逻辑
  - ✅ 从 `extractors/jtable.py` 迁移下载链接提取逻辑
  - ✅ 实现 `_extract_all_metadata()` 共享方法
  - ✅ 使用 MetadataBuilder 和 OperationItemBuilder
...
- **状态**: ❌ 不需要（JTable 不支持搜索功能）
- **说明**: 根据用户需求，JTable 插件不需要实现搜索功能，仅实现元数据提取和视频下载
