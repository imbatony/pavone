...
from ..core import Operator, HTTPDownloader, M3U8Downloader, DummyOperator
...
class ExecutionManager:
    ...
    def _execute_download(self, selected_item: OpertionItem, silent: bool = False, parent: Optional[OpertionItem] = None) -> bool:
        """
        执行下载或处理操作
    ...
    def _set_target_path_for_item(self, item: OpertionItem, parent_item: Optional[OpertionItem]) -> None:
        ...
        if parent_item is None:
            target_path = self._get_target_path_for_item(item)
            ...
            target_path = self._get_target_path_for_item(item, target_folder)
            item.set_target_path(target_path)


    def _get_target_path_for_item(self, item: OpertionItem, target_folder: Optional[str] = None) -> str:
        """
        获取操作项的目标路径
        Args:
            item: 操作项
        ...
        if not target_folder:
            ...
            if self.config.organize.auto_organize:
                ...
                target_sub_folder = item.get_target_subfolder(output_dir=output_dir, folder_name_pattern=folder_pattern)  # 获取子文件夹名称
                # 确保路径和文件名有效
        ...
        # 生成目标文件名
        if self.config.organize.auto_organize:
            file_name = item.get_file_name(file_name_pattern=naming_pattern)  # 获取文件名
        else:
        ...
        if file_name_suffix:
            file_name = f"{file_name}{file_name_suffix}"
    ...
    def download_from_url(self, 
        ...
        try:
            ...
            # 如果提供了文件名，则覆盖操作项的名称
            if file_name:
                selected_item.set_custom_filename(file_name)
    ...
    def batch_download(self, urls: List[str],
                      slient: bool = True) -> List[Tuple[str, bool]]:
        """
        批量下载多个URL
...
def create_exe_manager(
    config: Config,
    plugin_manager: Optional[PluginManager] = None) -> ExecutionManager:
    """
    创建执行管理器实例的便利函数
    Args:
        config: 配置对象
