...
class DownloadManager:
    ...
    def select_download_option(self, options: List[DownloadOpt]) -> DownloadOpt:
        ...
        if len(options) == 1:
            ...
            logger.info(f"找到1个下载选项: {options[0].get_full_description()}")
            return options[0]
        
        logger = get_logger(__name__)
        logger.info(f"找到 {len(options)} 个下载选项:")
        for i, opt in enumerate(options, 1):
            logger.info(f"  {i}. {opt.get_full_description()}")
        ...
        while True:
            try:
                ...
                if 1 <= choice_num <= len(options):
                    ...
                    logger = get_logger(__name__)
                    logger.info(f"已选择: {selected.get_full_description()}")
                    return selected
                else:
                    logger = get_logger(__name__)
                    logger.warning(f"请输入1到{len(options)}之间的数字")
    ...
    def download_from_url(self, url: str, 
        ...
        try:
            ...
            if auto_select:
                ...
                logger.info(f"自动选择: {selected_option.get_full_description()}")
            else:
            ...
            logger.info(f"使用 {downloader_type} 下载器")            
