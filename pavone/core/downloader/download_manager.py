...
class DownloadManager:
    ...
    def select_download_option(self, options: List[DownloadOpt]) -> DownloadOpt:
        ...
        """
        if len(options) == 1:
            self.logger.info(f"找到1个下载选项: {options[0].get_full_description()}")
            return options[0]
        
        self.logger.info(f"找到 {len(options)} 个下载选项:")
        for i, opt in enumerate(options, 1):
            self.logger.info(f"  {i}. {opt.get_full_description()}")
        ...
        while True:
            try:
                ...
                if 1 <= choice_num <= len(options):
                    ...
                    self.logger.info(f"已选择: {selected.get_full_description()}")
                    return selected
                else:
                    self.logger.warning(f"请输入1到{len(options)}之间的数字")
                    
            except ValueError as e:
                if "用户取消了下载" in str(e):
                    raise
                self.logger.warning("输入无效，请输入数字")
    ...
    def download_from_url(self, url: str, 
        ...
        """
        try:
            self.logger.info(f"正在分析URL: {url}")
            ...
            if auto_select:
                ...
                self.logger.info(f"自动选择: {selected_option.get_full_description()}")
            else:
            ...
            self.logger.info(f"使用 {downloader_type} 下载器")
    ...
    def download_option(self, option: DownloadOpt,
        ...
        try:
            ...
            logger.info(f"使用 {downloader_type} 下载器下载: {option.get_display_name()}")
