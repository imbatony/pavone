"""
Pacopacomama 元数据提取器测试

使用 Mock JSON API 响应
参考: metatube-sdk-go/provider/pacopacomama/pacopacomama_test.go
测试 ID: 011215_178
"""

from unittest.mock import MagicMock, patch

from pavone.plugins.metadata.pacopacomama_metadata import PacopacomamaMetadata


class TestPacopacomamaMetadata:
    def setup_method(self):
        self.extractor = PacopacomamaMetadata()

    def test_can_extract_url(self):
        assert self.extractor.can_extract("https://www.pacopacomama.com/movies/011215_178/")
    ...
    def _mock_json_response(self):
        ...
        mock.json.return_value = {
            "Title": "テスト動画タイトル",
            "ActressesJa": ["テスト花子", "テスト太郎"],
            "UCNAME": ["熟女", "素人"],
            "Desc": "テスト動画の説明です。",
            "Release": "2015-01-12",
            "Duration": 5400,
            "AvgRating": 3.5,
            "ThumbUltra": "https://www.pacopacomama.com/images/thumb_ultra.jpg",
            "MovieThumb": "https://www.pacopacomama.com/images/movie_thumb.jpg",
        }
    ...
    def test_extract_metadata_with_mock(self):
        ...
        with patch.object(self.extractor, "fetch", return_value=resp):
            metadata = self.extractor.extract_metadata("https://www.pacopacomama.com/movies/011215_178/")

        assert metadata is not None
        assert "011215_178" in metadata.code
        assert "テスト動画タイトル" in metadata.title
        assert "テスト花子" in metadata.actors
        assert metadata.studio == "パコパコママ"
    ...
    def test_extract_metadata_network_error(self):
        ...
        with patch.object(self.extractor, "fetch", side_effect=requests.ConnectionError("timeout")):
            result = self.extractor.extract_metadata("https://www.pacopacomama.com/movies/011215_178/")
